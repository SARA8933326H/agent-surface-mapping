...
import { AssetExtract, FormExtract, PageExtract, RiskDto } from '@surface/shared';
...
export class VulnService {
  ...
  async runChecks(targetUrl: string, pages: PageExtract[], forms: FormExtract[], assets: AssetExtract[]): Promise<RiskDto[]> {
    ...
    if (Config.VULN_ACTIVE_PROBES) {
      try {
        findings.push(...(await runActiveProbes(targetUrl, Config.VULN_PROBE_TIMEOUT)));
      } catch (error) {
